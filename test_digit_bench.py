...
from conf import create_stmt, run_all_bench, create_teardown, create_custom_stmt
...
S_SORT_STMT = "sorted_elements = ssort(elements, len(elements), max(elements), min(elements))"

ALL_STMT_TEST = {
    "S. sort (Only digits)": create_custom_stmt("ssort", load_file("sort/ssort/ssort.py"), S_SORT_STMT),
